...
export function ArtistLinksPanel() {
  ...
  async function save() {
    ...
    setError(null);
    try {
      await api.artistLinksSave({ groups: groups ?? [] });
      setFlash("saved ✓");
    } catch (e) {
      setError(String(e));
      await load();
  ...
  return (
    <div data-testid="artist-links-panel" className="h-full flex flex-col gap-3 p-3 overflow-auto">
      ...
      {error && (
        <div role="alert" className="text-danger text-xs px-3 py-2 rounded-md border border-danger/40">
          {error}
        </div>
      )}
          ...
          {groups.map((g, i) => (
            ...
            >
              <div className="flex-1 min-w-0">
                ...
                <div className="text-muted text-xs truncate">{g.members.join(", ")}</div>
              </div>
              <button
                type="button"
