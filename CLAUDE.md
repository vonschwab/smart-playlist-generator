...
- **Sonic space is MuQ (`X_sonic_muq`, 512-dim, `sonic_variant_override: muq`); the tower/MERT code paths were removed by SP-B (Tasks 1–8, 2026-07-01) — the artifact's legacy `X_sonic_mert*`/tower keys remain until the Task 10 rebuild, and the MERT source data is archived in Task 11; future embeddings slot in via the variant seam.** (`tower_weights`/`transition_weights` alignment and the 0.20/0.50/0.30 tower blend are historical — see Layer 3 items 17/18.)
