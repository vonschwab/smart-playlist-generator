...
17. **Tower PCA with weights rhythm 0.20 / timbre 0.50 / harmony 0.30** is our current best sonic decomposition (timbre dominant).
...
- **The 0.20/0.50/0.30 tower weighting is baked into the `tower_weighted` artifact at build time** (was previously inert under the shipped `robust_whiten` artifact — all three weighting mechanisms silently no-oped via dim-mismatch fallback). Rebuild via `scripts/rebuild_sonic_tower_weighted.py` after any artifact rebuild that resets the variant (e.g. a full re-analysis).
