...
- **The 0.20/0.50/0.30 tower weighting is baked into the `tower_weighted` artifact at build time.** The current production artifact uses **2DFTM harmony** (key-invariant 96-dim 2D Fourier Transform Magnitude, validated 2026-06-03) — blend is 163-dim (rhythm 10 + timbre 57 + harmony 96; rhythm PCA dim drifts 9↔10 by rebuild at 95% var, so 162↔163). If you ever re-run the full library analysis from scratch, re-fold via `scripts/fold_2dftm_into_artifact.py` (which also re-runs `scripts/extract_harmony_2dftm_sidecar.py` first). Note: the towers are now the *rollback* sonic space — the default is the learned MERT embedding (`X_sonic_variant: mert`); see the v6.0 changelog.
