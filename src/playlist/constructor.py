...
def construct_playlist(
    *,
    ...
    gap_values_np = np.array(gap_values, dtype=float) if gap_values else np.array([], dtype=float)
    ...
    if cfg.repair.enabled and len(order_array) > 2:
        ...
        repair_iters = 0
        unused_pool = [idx for idx in pool.pool_indices if idx not in current_order]
        while repair_iters < cfg.repair.max_iters:
            ...
            for edge in worst_edges:
                ...
                for cand in unused_pool:
                    # substitute_next
                    if cfg.repair.allow_substitute_next:
                    ...
                    # substitute_prev
                    if cfg.repair.allow_substitute_prev:
                        order_candidate = current_order.copy()
