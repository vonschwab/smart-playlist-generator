...
def stage_sonic(ctx: Dict) -> Dict:
    ...
    try:
        ...
        # Run the pool in-process. update_sonic.run()'s no-progress watchdog kills
        # a stalled pool and finishes serially, so the Windows spawn import
        # deadlock can never hang the run. (An earlier out-of-process attempt hung
        # in the child's interpreter startup on inherited GUI-worker stdio handles.)
        pipeline.run(
            limit=limit,
            workers=workers,
            force=force,
